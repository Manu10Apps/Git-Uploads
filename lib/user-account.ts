...
async function sendMail(to: string, subject: string, text: string, html: string) {
  ...
  if (!smtpHost || !smtpUser || !smtpPass) {
    console.warn('[user-email] SMTP not configured. Email was not sent.');
    console.info('[user-email] To:', to);
    console.info('[user-email] Subject:', subject);
    console.info('[user-email] Text:', text);
    return;
  }
...
export async function sendUserVerificationEmail(email: string, name: string, token: string) {
  ...
  const html = `
    <p>Hello ${name},</p>
    <p>Welcome to Intambwe Media! Click the button below to verify your email and activate your account:</p>
